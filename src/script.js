  ...
  window.addEventListener(
    ...
    () => {
      ...
      if (audioToggle.checked === true) {
        ...
        showDialog(
          "This URL is a valid of sound shader.\nIt is OK play the audio?",
          {
            okLabel: "yes",
            cancelLabel: "no",
          }
      ...
      download.addEventListener(
        ...
        () => {
          ...
          framerateInput.addEventListener(
            "change",
            () => {
              framerateInput.value = Math.min(
                Math.max(framerateInput.value, 10),
                60
              );
            },
            false
          ...
          qualityInput.addEventListener(
            "change",
            () => {
              qualityInput.value = Math.min(
                Math.max(qualityInput.value, 0),
                100
              );
            },
            false
          ...
          const radioListener = () => {
            const flag =
              typeRadioGif.checked === true || typeRadioWebM.checked === true;
          ...
          showDialog(wrap, { okLabel: "start" }).then((isOk) => {
            if (isOk !== true) {
              return;
            }
            if (
      ...
      link.addEventListener(
        "click",
        async () => {
          if (link.classList.contains("disabled") === true) {
            return;
          }
          ...
          const snapshotLink = await generateSnapshotLink().catch((error) => {
            console.error(error);
            alert(
              "Failed to create a snapshot. See the console for more info."
            );
            ...
            switch (broadcastMode) {
              ...
              case "audience":
                if (channelData.disc !== "unknown") {
                  ...
                  showDialog(
                    "This channel is a valid of sound shader.\nIt is OK play the audio?",
                    {
                      okLabel: "yes",
                      cancelLabel: "no",
                    }
